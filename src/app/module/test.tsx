...
import { FormData, LocationData, CombinedData } from '@/app/types/login';
...
const LoginForm = () => {
  ...
  const fetchCombinedData = useCallback(async () => {
    ...
    try {
      ...
      const authorizedUsers = data?.userRoles[0]?.portals?.[0]?.screens || [];
  
      // Check if homepage exists in authorized screens
      if (!authorizedUsers.some((screen: { value: string }) => screen.value.toLowerCase() === "homepage")) {
